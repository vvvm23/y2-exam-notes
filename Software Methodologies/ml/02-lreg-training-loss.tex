...
\textbf{Batch Gradient Descent} -- Use the entire dataset each step.

\textbf{Stochastic Gradient Descent} - One example at a time. This leads to random movements and so the final values will be good but not optimal.

\textit{This randomness can help with escaping local minimas.}
...
\textit{One compromise is to gradually reduce the learning rate over time.}
...
Sometimes the data will be far more complex than a simple straight plane. We can actually use a linear model to fit non-linear data. To do this, we can create additional features that are the square (or higher degree) of the source feature as new features.

We can now fit the linear model with these new features to fit non-linear data.

We can also add combinations of features, such as $ab$, $a^2b$, etc. in order to find relationships between features.
